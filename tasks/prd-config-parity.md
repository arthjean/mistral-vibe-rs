...
- [ ] Given the popular set, when fields are described, then exactly the 12 reference entries are marked popular (the PRD said 13; the pinned `POPULAR_SETTINGS` holds 12, which the committed corpus already records)
...
| Every config response is enveloped as `snapshot` rather than `runtime` plus `config` | Predates this PRD: `config/read`, `config/reload` and `config/batchWrite` already answer with `ConfigSnapshot::public_view`, and `config/patch` follows them rather than splitting the envelope in two |
| `config/patch` adds `changedKeys` to `rejected` and `failures` | The keys the write moved, read off the change bus US-069 adds. A settings screen refreshes what moved instead of re-reading the surface; the reference client re-reads |
| `config/patch` accepts `reloadRuntime` and ignores it | `config/read` and `config/reload` both compose from disk on every call in this port, so no cached runtime can be left stale. Recorded rather than dropped, so the reference request shape still validates |
