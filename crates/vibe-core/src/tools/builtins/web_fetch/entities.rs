//! The HTML5 named character references and the numeric-reference fixups the
//! two HTML readers `web_fetch` reproduces apply.
//!
//! Generated from CPython 3.12 `html.entities.html5`, `html._invalid_charrefs`
//! and `html._invalid_codepoints`, and from Beautiful Soup 4.14
//! `UnicodeDammit.WINDOWS_1252_TO_UTF8`: the WHATWG entity list and the
//! Windows-1252 fallback its numeric-reference state defines. Regenerate from
//! the reference environment rather than editing by hand.

/// Every named reference, keyed as `html.entities.html5` keys it: with its
/// semicolon, and also without one for the legacy names HTML allows bare.
/// Sorted by key, so a lookup is a binary search.
pub(super) const NAMED: &[(&str, &str)] = &[
    ("AElig", "\u{c6}"),
    ("AElig;", "\u{c6}"),
    ("AMP", "&"),
    ("AMP;", "&"),
...
/// `html._invalid_charrefs`: numeric references `html.unescape` remaps.
pub(super) const INVALID_CHARREFS: &[(u32, &str)] = &[
    (0x0, "\u{fffd}"),
    (0xd, "\u{d}"),
    (0x80, "\u{20ac}"),
    (0x81, "\u{81}"),
...
/// `html._invalid_codepoints`: numeric references `html.unescape` drops.
pub(super) const INVALID_CODEPOINTS: &[u32] = &[
...
/// `UnicodeDammit.WINDOWS_1252_TO_UTF8` over the C1 range, which Beautiful
/// Soup substitutes for a numeric reference in 0x80 to 0x9F.
pub(super) const WINDOWS_1252: &[(u32, &str)] = &[
    (0x80, "\u{20ac}"),
    (0x82, "\u{201a}"),
    (0x83, "\u{192}"),
    (0x84, "\u{201e}"),
