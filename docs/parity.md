...
| Last remeasure | 2026-08-10, from the skills oracle. Only the skills row was restated, from 55 to 100 against the counts `skills_parity_tests` prints (209 scenarios across 8 families, 0 divergent outside the named ledger), with the rank-11 execution-order row closed and the accepted-divergences table gaining the builtin prose, store fallback prose and dormant-registry rows; every other part carries the number its own remeasure produced, so the weighted total below is a hand-maintained judgement over rows measured at different dates. The previous remeasure, 2026-08-08 from the compaction oracle, restated compaction to 100 and amended the configuration rows for the five keys it consumes |
...
| Weighted score | 78/100 (was 77 at the last remeasure, 65 at first audit) |
