...
| Last remeasure | 2026-08-08, at the `b78b451` re-pin. Only the app-server row was restated from the new counts, and the configuration row's residual amended; every other part carries the number its own remeasure produced, so the weighted total below predates all of them |
