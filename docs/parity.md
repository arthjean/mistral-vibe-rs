...
| Trusted folders and permissions | 85 | `policy.rs`: modes, rules, leases, trust roots, approvals. Each tool's configured `permission`, `allowlist`, `denylist` and `sensitive_patterns` now decide alongside them, with the session permission override outranking the file. The scope vocabulary is still this port's own, which EP-032 replaces |
